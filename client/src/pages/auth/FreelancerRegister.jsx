...
function FreelancerRegister() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await authService.registerFreelancer({
        name,
        email,
        password,
        confirmPassword,
      ...
      if (res.success) {
        showToast('Freelancer account registered successfully!', 'success')
        setTimeout(() => {
          navigate('/login')
        }, 1500)
      } else {
        ...
        <Card className="border-slate-200">
          <div className="mb-6">
            <h2 className="text-base font-bold text-[#0F172A] flex items-center gap-2">
              ...
              <span>Freelancer Register</span>
            </h2>
            <p className="text-xs text-[#64748B] mt-0.5">Bid on jobs and configure your profile cards</p>
          </div>
